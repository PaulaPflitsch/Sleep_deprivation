function [meanAct, sumAct, cumsumAct, tg, wt, allWells] = fct_crNormalDistr(a, expName, geno, allWells,  Group1WT, Group2TG)
...
fct_suptitle([' Overview activity Group1WT: ' expName]);
...
% figure('Color',[1 1 1]);
% colormap('hot'); % default bone gray hot hsv jet winter copper pink
% ax=imagesc(a(:,Group2TG)');
% % set(ax,'box','off');
% % colorbar
% xlabel('time in seconds');
% ylabel('individual fish, activity white');
% title('Activity "heatmap"');
% fct_suptitle([' Overview activity Group2TG: ' expName]);
% % datetick('x','HHPM');
...
indices = find(meanAct(:,:)<1); %set a threshold (of 1) for the mean activity to exclude dead fish
meanAct(indices) = NaN; %exclude everything below the threshold
sumAct=sum(a(:,allWells)); % summed active seconds each fish for whole experiment, single row vect.
indices = find(sumAct(:,:)<3)%work on this threshold!!
...
format long;
clear meanAct xlabel ylabel title sumAct; 
meanAct = mean(a(:,Group1WT));  % mean of active seconds for each fish for whole experiment, single row vector
sumAct=sum(a(:,Group1WT)); % summed active seconds each fish for whole experiment, single row vect.

% Mean Fish activities overview
figure('Color',[1 1 1])  % make figure with white backround
subplot(2,3,1) % make figure with 6 panels
plot(mean(a(:,Group1WT),2)); % mean(a,2) gives mean of rows = mean for each time point
...
% format long;
% clear meanAct xlabel ylabel title sumAct; 
% meanAct = mean(a(:,Group2TG));  % mean of active seconds for each fish for whole experiment, single row vector
% sumAct=sum(a(:,Group2TG)); % summed active seconds each fish for whole experiment, single row vect.
% 
% % Mean Fish activities overview
% figure('Color',[1 1 1])  % make figure with white backround
% subplot(2,3,1) % make figure with 6 panels
% plot(mean(a(:,Group2TG),2)); % mean(a,2) gives mean of rows = mean for each time point
% 
% % Create xlabel
% xlabel('Minutes','Interpreter','none');
% % Create ylabel
