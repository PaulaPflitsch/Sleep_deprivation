function [activity] = fct_crBinActivity(d24_clip,b,threshold, framerate)
...
% not start with zero?)
startBins = 1:binInterval:length(d24_clip); % frame number of first frame in each bin
...
activity=zeros(numSlices,size(d24_clip,2)); 
...
for i = 1:length(startBins)
    ...
    sliver = d24_clip(startBins(i):startBins(i)+binInterval-1,1:size(d24_clip,2));
